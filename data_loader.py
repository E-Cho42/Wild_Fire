...
def make_bounding_box(lat, long, size_km = 10):
...
def download_elevation(lat,long,size_km=10):
    if is_in_colorado(lat=lat, long = long):
        
        bounds = make_bounding_box(lat,long,size_km=10)
        bbox = (bounds["west"], bounds["south"], bounds["east"], bounds["north"])
        
        dataset = elevatr.get_elev_raster( locations=bbox, crs=pyproj.CRS.from_epsg(4326), zoom=11)
